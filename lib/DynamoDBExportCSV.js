...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
    ...
    function writeTableToCsv(table, columns, totalSegments, segment, compressed, callback) {
        var query = {
            TableName: table,
            Segment: segment,
        ...
        async.doWhilst(
            function(done) {
                ...
                var writableStream = fs.createWriteStream(table + '/' + fileName);
                ...
                async.doWhilst(
                    ...
                    function() {
                        ...
                        return query.ExclusiveStartKey && writableStream.bytesWritten < 1024 * 1024 * 250;
                    },
                    function(err) {
                        if(err)
                        {
    ...
    this.exportTable = function(table, columns, totalSegments, compressed, callback) {
        async.series([
            ...
            function(done) {
                ...
                {
                    parallelScanFunctions.push((function(segment) { return function(done) {
                        writeTableToCsv(table, columns, totalSegments, segment, compressed, done);
                    }}(i)));
                }
