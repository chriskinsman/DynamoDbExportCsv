...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
    ...
    function writeTableToCsv(table, columns, totalSegments, segment, compressed, filesize, callback) {
        var query = {
            TableName: table,
            Segment: segment,
    ...
    this.exportTable = function(table, columns, totalSegments, compressed, filesize, callback) {
        if(!filesize)
        {
            filesize = 250;
        }
            ...
            function(done) {
                ...
                for(var i = 0; i < totalSegments; i++)
                {
                    parallelScanFunctions.push((function(segment) { return function(done) {
                        writeTableToCsv(table, columns, totalSegments, segment, compressed, filesize, done);
                    }}(i)));
                }
