...
function DynamoDBExportCSV(awsAccessKeyId, awsSecretAccessKey, awsRegion) {
    ...
    function getItemWithoutHeaders(item, callback) {
        var row = {};
    ...
    // is represented
    function getItemWithHeaders(item, columns, callback) {
        var row = {};
                            ...
                            if (err) {
                                ...
                                if (data) {
                                    ...
                                    async.eachSeries(data.Items, function (item, done) {
                                        if (fileRowCount === 0) {
                                            row = getItemWithHeaders(item, columns, done);
                                        }
                                        else {
                                            row = getItemWithoutHeaders(item, done);
                                        }

                                        noDrainRequired = csvStream.write(row, callback);
                                        fileRowCount++;
                                        rowCount++;
